...
export default function HeaderNavbarFull(props: {history: History<any>}){
    ...
    <NavLink className="underline font-bold z-20    " to={a.pathWrapper.path || a.getPathFromArgs({})} activeClassName="no-underline text-boathouseblue" exact={true} onClick={e => {
        e.preventDefault();
    ...
    return <span className="hidden lg:block relative z-1"><hr className="border-t-1 border-black mb-[2px] mt-[5px]"/><nav className="bg-card pl-card rounded-b-md">{navOptions}</nav></span>;
}
