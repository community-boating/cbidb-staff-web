...
import {
	...
	StringifiedProps,
	stringify,
	stringifyAndMakeBlank,
} from "@util/StringifyObjectProps";
	...
	const submit = () => {
		...
		return new Promise((resolve, reject) => {
			...
			window.setTimeout(() => {
				...
				const candidateForValidation = destringify(
					props.rowValidator,
					nullifyEmptyStrings(formData.rowForEdit),
					false,
					props.primaryKey
				);
				...
				const pkValue =
					candidateForValidation[props.primaryKey] === null
						? -1
						: candidateForValidation[props.primaryKey];
					...
					props.submitRow.send(makePostJSON(toSend)).then((ret) => {
						if (ret.type == "Success") {
							...
							if (
								rowData.find(
									(r) =>
										String(r[props.primaryKey]) ==
										formData.rowForEdit[props.primaryKey]
								)
							) {
			...
			<Card className={props.cardClassName ?? ""}>
				<CardHeader>
					...
					>
						...
						{props.addButtonLocation === "header" && (
							<a
								href=""
								onClick={(e) => {
									e.preventDefault();
				...
				<CardBody>
					<div>
						<SimpleReport
							...
							bootstrap4
							bordered={false}
							sizePerPage={props.sizePerPage ?? 12}
							sizePerPageList={
						...
						/>
						{props.addButtonLocation === "footer" ||
							(props.addButtonLocation === undefined && (
								<Button
									className="mr-1 mb-1"
									outline
